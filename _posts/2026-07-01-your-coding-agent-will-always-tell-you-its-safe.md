...
So trust can't come from the agent's account of itself. It has to come from outside the agent entirely — and the watcher only earns that seat if it can't become the next trust problem. Read everything, change nothing, send nothing, and be simple enough to audit. That's the spec — verifiability made concrete: evidence about the agent that the agent can't touch.
